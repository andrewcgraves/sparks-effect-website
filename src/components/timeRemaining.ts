...
// The starting location is a place the rider picked rather than a station, so
// it has no slug of its own.
export const ORIGIN_KEY = 'origin'

// Lanes narrow as they multiply so that station names keep their room, down to
// a floor past which they stop shrinking and the column takes the extra width
// from the names instead — which only a graph branching nine lanes deep can
// reach, and is the better of two bad answers, since lanes thinner than the
// floor cannot be told apart anyway.
export const GRAPH_COLUMN_PX = 96
export const MAX_LANE_PX = 20
export const MIN_LANE_PX = 10

export interface RowDetail {
  // Shown only when it differs from the moment they leave — the difference
  // being the dwell served here plus any boarding wait paid here.
  arrivalSecs?: number
  dwellSecs?: number
  rideSecs?: number
...
export interface TimeRemainingRow {
  ...
  slug: string | null
  label: string
  // The wire reports the moment they arrive, so any wait paid here has already
  // been taken off.
  remainingSecs: number
  // Null where the journey ends, and that absence is the thing that says so.
  flag: string | null
  parentKey: string | null
  detail: RowDetail
  // A fork onto its own lane is the branch carrying straight on.
  lane: number
  through: number[]
  forks: number[]
  // False only for the starting location, which nothing leads to.
  incoming: boolean
}

// Offered only when there is no line to read instead. See viewMemberships.
...
// A span of time inside a trip rather than what is left of the budget. It
// keeps the seconds below a minute, because the things it describes are
...
// The wire reports the moment they arrive, and a rider who boards here leaves
// the boarding wait later. The row's own value and the order the rows are
// sorted into are the same number, and come from here so they cannot drift
// apart.
...
interface Trip {
  ...
  context: TimeRemainingContext
  // Both are read per view rather than per station — see progressFor.
...
// One view per line, ordered by the first station each reaches, so the line
// that gets the rider furthest is offered first.
//
...
// Re-rooted onto the starting location where the row they came from is not in
// this view.
...
// Mutates the rows with the geometry it works out.
