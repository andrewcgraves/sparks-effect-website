...
// The seeded one carries a scenario_slug alongside these; the authored ones
// name their target in the URL instead, which is the only difference between
// the three bodies — so one enqueue serves all three.
...
// Matches the compile poll: fast enough that a result does not sit unseen,
// slow enough to be unremarkable.
const POLL_INTERVAL_MS = 1000

// Measured end to end: it starts before the request is even sent, not once
// there is a job to poll.
//
...
// Owned here rather than taken from the server's own prose, the same way an
// out-of-range origin's message is composed client-side from its code: the
// API's wording is the fallback for callers that are not this app.
//
...
// A caller can chain this into its error handling the way it already chains
// outOfRangeError.
//
...
// `init` is how an enqueue reuses its trace id on every poll (SPA-205);
// one-shot callers omit it and get a fresh id from apiRequest.
...
// Turns a 202 back into the promise-of-a-result the callers above still
// expect, so nothing outside this layer has to know an isochrone is now
...
// Spends what is left of the deadline that started at `startedAt` — so a slow
// enqueue eats into the wait rather than being granted a fresh one on top of it.
