...
// `init` is how a compile reuses its trace id on every poll (SPA-205);
// one-shot callers omit it.
...
// Resolves with the succeeded job itself — its `result` is the compiled
// TransitGraph. There is no separate fetch-by-slug: the compile endpoints
// embed the result directly on the job, so a caller already reading the graph
// reads it off this return value.
