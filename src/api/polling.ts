...
// `jobError` is the API's own `error` field, kept apart from `message` (which
// wraps it with the job id for a log line) so a caller that has somewhere to
// show the user a reason — the isochrone form's error banner, for one
// (SPA-230) — can show exactly what the API said rather than inventing its
// own wording or a caller having to re-parse `message` to get it back out.
...
// Resolves with the job itself rather than just its result, because callers
